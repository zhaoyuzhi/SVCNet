...
if __name__ == "__main__":
    ...
    parser.add_argument('--baseroot', type = str, \
        default = './test_data/input', \
            help = 'baseroot')
    parser.add_argument('--scribbleroot', type = str, \
        default = '00000_256p_not_widen.png', \
            help = 'scribbleroot, based on one given scribble image')
    parser.add_argument('--saveroot', type = str, \
        default = "./test_data/color_point40_color_width5_256p", \
            help = 'saveroot')
    ...
    imglist = sorted(get_files(opt.baseroot))
    print(imglist)
    ...
    for i in range(N):
        ...
        if i == 0:
            if opt.scribbleroot: # if there is a given color scribble image
                s = Image.open(opt.scribbleroot).convert('RGB')
                s = np.array(s)
        ...
        save_sub_folder_name = opt.saveroot
        check_path(save_sub_folder_name)

        save_resized_scribble_name = os.path.join(save_sub_folder_name, imgpath.split('\\')[-1].split('.')[0] + '_' + str(opt.crop_size_h) + 'p.png')
        widen_s = cv2.cvtColor(widen_s, cv2.COLOR_BGR2RGB)
        cv2.imwrite(save_resized_scribble_name, widen_s)

        save_original_size_scribble_name = os.path.join(save_sub_folder_name, imgpath.split('\\')[-1].split('.')[0] + '.png')
        ...
        save_resized_scribble_name = os.path.join(save_sub_folder_name, imgpath.split('\\')[-1].split('.')[0] + '_' + str(opt.crop_size_h) + 'p_not_widen.png')
        cv2.imwrite(save_resized_scribble_name, s)

        save_original_size_scribble_name = os.path.join(save_sub_folder_name, imgpath.split('\\')[-1].split('.')[0] + '_not_widen.png')
