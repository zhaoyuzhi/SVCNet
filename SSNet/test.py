...
def read_img_path(imglist, i, opt):
...
def save_img(tensor_L, tensor_ab, save_name, opt):
    ...
    save_rgb_name = os.path.join(save_rgb_sub_folder_name, save_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scribble_root', type = str, \
        default = './test_data/color_point40_color_width5_256p_diverse1', \
            help = 'the base testing folder')
    ...
    # Define the dataset
    imglist = get_filespath(opt.base_root)
    ...
    for i in range(len(imglist) - opt.iter_frames + 1):
        
        # Define paths, the output should contain opt.iter_frames paths
        img_paths, scribble_paths = read_img_path(imglist, i, opt)
